...
public:
    ...
    ssv propMu  (const ssv &xtm1, const csv &cov_data, const psv& untrans_old_param);    
    ssv q1Samp (const osv &y1, const psv& untrans_p1);    
    ssv fSamp (const ssv &xtm1, const csv &zt, const psv& untrans_new_param); 
...
template<size_t nparts, typename float_t>
svol_lw_1_from_csv<nparts,float_t>::svol_lw_1_from_csv(const float_t& delta, const std::string &csv_of_samples)
    : LWFilterWithCovs<nparts,dimx,dimy,dimcov,dimparam,float_t>(
    		std::vector<std::string> {"logit", "null", "log", "twice_fisher"}, // phi, mu, sigma, rho
            	delta)           // PRIORS
    // REMINDER: does output appear to be extremely sensitive to these?
...
template<size_t nparts, typename float_t>
auto svol_lw_1_from_csv<nparts,float_t>::propMu(const ssv &xtm1, const csv &cov_data, const psv& untrans_old_param) -> ssv
{
    // phi, mu, sigma, rho
    return untrans_old_param(1) + untrans_old_param(0)*(xtm1 - untrans_old_param(1)) + cov_data(0)*untrans_old_param(3)*untrans_old_param(2)*std::exp(-.5*xtm1); 
}
...
template<size_t nparts, typename float_t>
auto svol_lw_1_from_csv<nparts,float_t>::fSamp(const ssv &xtm1, const csv &zt, const psv& untrans_new_param) -> ssv
{
    // phi, mu, sigma, rho
    ssv xt;
    float_t mean = untrans_new_param(1) + untrans_new_param(0)*(xtm1(0) - untrans_new_param(1)) + zt(0)*untrans_new_param(3)*untrans_new_param(2)*std::exp(-.5*xtm1(0));
...
template<size_t nparts, typename float_t>
svol_lw_2_from_csv<nparts,float_t>::svol_lw_2_from_csv(const float_t& delta, const std::string &csv_param_samples)
    : LWFilter2WithCovs<nparts,dimx,dimy,dimcov,dimparam,float_t>(
    				std::vector<std::string>{"logit", "null", "log", "twice_fisher"}, // phi, mu, sigma, rho
            			delta)           // PRIORS    // REMINDER: does output appear to be extremely sensitive to these?
    // REMINDER: does output appear to be extremely sensitive to these?
...
template<size_t nparts, typename float_t>
auto svol_lw_2_from_csv<nparts,float_t>::qSamp(const ssv &xtm1, const osv &yt, const csv &cov_data, const psv& untrans_pt) -> ssv
{
    // phi, mu, sigma, rho
    ssv xt;
    float_t mean = untrans_pt(1) + untrans_pt(0)*(xtm1(0) - untrans_pt(1)) + cov_data(0)*untrans_pt(3)*untrans_pt(2)*std::exp(-.5*xtm1(0));
...
template<size_t nparts, typename float_t>
float_t svol_lw_2_from_csv<nparts,float_t>::logQEv(const ssv &xt, const ssv &xtm1, const osv &yt, const csv &cov_data, const psv& untrans_pt)
{
    // phi, mu, sigma, rho
    float_t mean = untrans_pt(1) + untrans_pt(0)*(xtm1(0) - untrans_pt(1)) + cov_data(0)*untrans_pt(3)*untrans_pt(2)*std::exp(-.5*xtm1(0));
    float_t sd = untrans_pt(2) * std::sqrt( 1.0 - untrans_pt(3) * untrans_pt(3));
...
public:
    ...
    // ctor
    // make sure parameter samples in csv are for phi,mu,sigma,rho
    svol_swarm_2(const std::string &param_csv_filename, const std::vector<state_parm_func>& fs)
        : SwarmBase(fs)
    	, m_param_sampler(param_csv_filename)
    {
    }
