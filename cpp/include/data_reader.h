...
std::vector<Eigen::Matrix<float_t,Eigen::Dynamic,1> > readInData(const std::string &fileLoc, char delim)
{
   
    // build this up and return it
    std::vector<Eigen::Matrix<float_t,Eigen::Dynamic,1> > data;
    ...
    while(std::getline(ifs, line)){
        ...
        Eigen::Map<Eigen::Matrix<float_t,Eigen::Dynamic,1>> drw(&data_row[0], num_col);
        data.push_back(drw);
    }
